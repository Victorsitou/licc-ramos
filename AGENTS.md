...
- Auth: JWT via `jose` in an httpOnly cookie named `token` (7-day expiry). `getCurrentUser()` (src/lib/auth.ts) reads the cookie; API routes wrap handlers with `withAdmin` / `withVerified` (app/api/wrappers/) which re-fetch the user from the DB and check `verified`/`role`. Passwords: bcryptjs, 12 rounds.
