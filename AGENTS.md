...
- Auth: JWT via `jose` in httpOnly cookies: `token` (access, 15-min expiry) + `refreshToken` (opaque, 30 days, stored SHA-256-hashed in the `RefreshToken` Prisma table, rotated on each use, revoked on logout/password change). `getCurrentUserWithRefresh()` (src/lib/auth.ts) silently rotates an expired access token via the refresh token; API routes wrap handlers with `withAdmin` / `withVerified` (app/api/wrappers/) which re-fetch the user from the DB, check `verified`/`role`, and attach rotated cookies via `attachRotatedCookies`. `POST /api/auth/refresh` rotates and reissues the pair. Client calls use `authedFetch` (app/services/api.ts) which retries once after a 401 + refresh. Passwords: bcryptjs, 12 rounds.
